...
export const triggerMintingSqs = {
  ...
  events: [
    {
      sqs: {
        arn: '${self:provider.environment.AWS_SQS_LAMBDA_SCHEDULE_MINTING_ARN}',
        batchSize: 1,
      },
    },
  ],
...
export const triggerProjectMintingSqs = {
  handler: `${handlerPath(__dirname)}/handler.TriggerProjectMintingFn`,
  events: [
    {
      sqs: {
        arn: '${self:provider.environment.AWS_SQS_LAMBDA_PROJECT_MINTING_ARN}',
        batchSize: 1,
      },
    },
  ],
